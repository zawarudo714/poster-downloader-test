...
(function () {
  ...
  //
  // The catalogue's description column holds exactly seventeen words —
  // city, town, mountain, island, lake, castle, protected area, monument,
  // national park, cathedral, bridge, World Heritage Site, waterfall,
  // lighthouse, cave, beach, desert. Every one gets a drawing here.
  //
  // THEY ARE DRAWINGS, NOT EMOJI, AND THAT IS THE POINT. Emoji come with
  // their own colours and their own house styles, so seventeen of them
  // beside each other look like seventeen unrelated stickers. These are all
  // the same weight of line, all `currentColor`, so the page's own colour
  // decides how they look and they read as one family.
  //
  // Unknown words fall back to a map pin rather than to nothing: a project
  // whose sheet uses different words still gets a sensible strip, and the
  // WORD is always shown beside the picture anyway.
  const SUBJECT_ICONS = {
  ...
  function subjectIcon(kind) {
    const paths = SUBJECT_ICONS[String(kind || '').trim().toLowerCase()]
               || SUBJECT_ICONS[String(kind || '').trim()]
               || SUBJECT_FALLBACK;
